...
export default function UniversalTable<T>({
  ...
  const filteredData = useMemo(() => {
    ...
    const filterKey = filters.options.find(
      (opt) => opt.value === filters.value
    )?.value;
    ...
    return data.filter((row: T) => {
      ...
      return Object.values(row as Record<string, unknown>).some(
        (val) =>
          typeof val === "string" &&
          val.toLowerCase() === filters.value.toLowerCase()
      );
    });
  }, [data, filters]);
  ...
  if (loading) {
    return (
      <div
        className={`bg-white border border-black/20 rounded-xl p-6 ${className}`}
      >
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <tr key={headerGroup.id}>
                {headerGroup.headers.map((header) => (
                  <th
                    key={header.id}
                    className="px-3 py-3.5 text-left text-base font-bold text-gray-900 cursor-pointer hover:bg-gray-100 border-b border-gray-300"

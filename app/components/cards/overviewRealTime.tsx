import React from "react";

const OverviewRealTime = ({title, quantity, description}:{title: string, quantity: number|string, description: string}) => {
  return (
    <div className="group w-full">

    <div className="group-hover:bg-[#014852] text-black group-hover:text-white text-xl font-bold p-5 rounded-xl w-full flex flex-col gap-5 border-[1px] border-black/20 cursor-pointer ">
      <div className="flex items-center justify-between">
        <span>{title}</span>
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 512 512"
          className="size-6 fill-black group-hover:fill-white"
        >
          <path d="M135.2 117.4l-26.1 74.6 293.8 0-26.1-74.6C372.3 104.6 360.2 96 346.6 96L165.4 96c-13.6 0-25.7 8.6-30.2 21.4zM39.6 196.8L74.8 96.3C88.3 57.8 124.6 32 165.4 32l181.2 0c40.8 0 77.1 25.8 90.6 64.3l35.2 100.5c23.2 9.6 39.6 32.5 39.6 59.2l0 192c0 17.7-14.3 32-32 32l-32 0c-17.7 0-32-14.3-32-32l0-32-320 0 0 32c0 17.7-14.3 32-32 32l-32 0c-17.7 0-32-14.3-32-32L0 256c0-26.7 16.4-49.6 39.6-59.2zM128 304a32 32 0 1 0 -64 0 32 32 0 1 0 64 0zm288 32a32 32 0 1 0 0-64 32 32 0 1 0 0 64z" />
        </svg>
      </div>
      <div>
        <div className="flex flex-col">
          <span className="text-2xl">{quantity}</span>
          <span className="text-base font-normal group-hover:text-white/60 text-black/50">{description}</span>
        </div>
      </div>
    </div>
    </div>
  );
};

export default OverviewRealTime;

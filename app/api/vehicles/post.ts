...
export async function postVehicle(request: NextRequest) {
    ...
    if(!body.manufacturer || !body.model || !body.vehicleRegNo || !body.vin || !body.color){
        return NextResponse.json({
            message: "Missing required fields: manufacturer, model, vehicle registration number, VIN, and color are required"
        },{status: 400})
    }
    ...
    try {
        ...
        return NextResponse.json({message: "Failed to add a vehicle: " + (error as Error).message},{status: 500})
    }
}
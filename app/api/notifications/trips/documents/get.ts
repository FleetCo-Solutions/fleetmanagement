...
import { tripDocuments, notificationTopics } from "@/app/db/schema";
...
export async function checkExpiringDocuments() {
  try {
    ...
    const docs = await db.query.tripDocuments.findMany({
      where: isNull(tripDocuments.deletedAt),
      with: {
        trip: true,
      },
    });
    ...
    for (const doc of docs) {
      ...
      if (checkDocumentExpiryStatus(expiryDate)) {
        const companyId = doc.trip.companyId || "";
        ...
        const message = `The "${doc.title}" for trip #${doc.trip.id.slice(0, 8)} ${getExpiryStatus(daysRemaining)} (${formatExpiryDate(expiryDate)}). Please take action.`;
    ...
    if (count === 0) {
      return NextResponse.json({
        success: true,
        message: "No trip expiring documents",
      });
    }

    return NextResponse.json({
      success: true,
      message: `Processed ${count} trip expiring documents`,

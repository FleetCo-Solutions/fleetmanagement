...
import { driverDocuments, notificationTopics } from "@/app/db/schema";
...
export async function checkExpiringDocuments() {
  try {
    ...
    const docs = await db.query.driverDocuments.findMany({
      where: isNull(driverDocuments.deletedAt),
      with: {
        driver: true,
      },
    });
    ...
    for (const doc of docs) {
      ...
      if (checkDocumentExpiryStatus(expiryDate)) {
        const companyId = doc.driver.companyId || "";
        ...
        const message = `The "${doc.title}" for driver ${doc.driver.firstName} ${doc.driver.lastName} ${getExpiryStatus(daysRemaining)} (${formatExpiryDate(expiryDate)}). Please take action.`;
    ...
    if (count === 0) {
      return NextResponse.json({
        success: true,
        message: "No driver expiring documents",
      });
    }

    return NextResponse.json({
      success: true,
      message: `Processed ${count} driver expiring documents`,

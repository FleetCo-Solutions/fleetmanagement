...
import { vehicleDocuments, notificationTopics } from "@/app/db/schema";
...
export async function checkExpiringDocuments() {
  try {
    ...
    console.log("This are users by company", usersByCompany);
    ...
    console.log("This are documents", docs);
    let count = 0;
    ...
    for (const doc of docs) {
      ...
      if (checkDocumentExpiryStatus(expiryDate)) {
        const companyId = doc.vehicle.companyId || "";
        ...
        const message = `The "${doc.title}" for vehicle ${doc.vehicle.registrationNumber} ${getExpiryStatus(daysRemaining)} (${formatExpiryDate(expiryDate)}). Please take action.`;
    ...
    if (count === 0) {
      return NextResponse.json({
        success: true,
        message: "No vehicle expiring documents",
      });
    }

    return NextResponse.json({
      success: true,
      message: `Processed ${count} expiring documents`,

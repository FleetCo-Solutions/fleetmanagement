...
import {
  ...
  useDriverDocumentsSummaryQuery,
...
export default function DocumentsTab({ driverId }: { driverId: string }) {
  const { data: summary, isLoading: isSummaryLoading } =
    useDriverDocumentsSummaryQuery(driverId);
  const { data: documents, isLoading: isDocsLoading } =
    useDriverDocumentsQuery(driverId);
  const { mutateAsync: deleteDoc } = useDeleteDriverDocumentMutation(driverId);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-black/20 rounded-xl p-6 shadow-sm">
        <h3 className="text-xl font-bold text-black mb-4">Driver Documents</h3>

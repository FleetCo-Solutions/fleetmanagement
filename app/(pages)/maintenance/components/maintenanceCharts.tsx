...
const MaintenanceCharts = ({ type }: MaintenanceChartsProps) => {
  ...
  const healthDistribution = useMemo(() => {
    ...
    const good = maintenanceData.filter(
      (v) => v.healthScore >= 70 && v.healthScore < 90
    ).length;
    const fair = maintenanceData.filter(
      (v) => v.healthScore >= 50 && v.healthScore < 70
    ).length;
    ...
    <div className="flex flex-col gap-8 w-full">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="bg-white border border-black/20 rounded-xl p-6 md:w-[40%] w-full">
          <span className="text-xl font-bold mb-4 text-black block">
            Vehicle Health Distribution
          </span>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>

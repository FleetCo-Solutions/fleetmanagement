...
import {
  ...
  useVehicleDocumentsSummaryQuery,
...
export default function DocumentsTab({ vehicleId }: { vehicleId: string }) {
  const { data: summary, isLoading: isSummaryLoading } =
    useVehicleDocumentsSummaryQuery(vehicleId);
  const { data: documents, isLoading: isDocsLoading } =
    useVehicleDocumentsQuery(vehicleId);
  const { mutateAsync: deleteDoc } =
    useDeleteVehicleDocumentMutation(vehicleId);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-black/20 rounded-xl p-6 shadow-sm">
        <h3 className="text-xl font-bold text-black mb-4">Vehicle Documents</h3>

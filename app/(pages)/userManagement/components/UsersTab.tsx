...
import { useToggleUserStatus, useUserQuery } from "../query";
...
const UsersTab = () => {
  ...
  const columns: ColumnDef<BackendUser>[] = [
    ...
    },
    {
      ...
      accessorKey: "phone",
      cell: ({ row }) => (
        <span className="font-semibold text-black">{row.original.phone}</span>
      ),
    },
    {
      header: "Email",
    ...
    // },
    {
      ...
      cell: ({ row }) => (
        ...
        >
          {(row.original.status || "active").charAt(0).toUpperCase() +
            (row.original.status || "active").slice(1)}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Link
          href={`/userManagement/edit?id=${row.original.id}`}
  ...
  const changeUserStatus = async(user: User) => {
    await toast.promise(
      toggleUserStatus({ id: user.id, status: user.status === 'active' ? 'inactive' : 'active' }),
      {
        loading: "Toggling user status...",
        success: (data) => data.message || "User status successfully changed.",
  ...
  //       </svg>
  //     ),
  //     onClick: (row: User) => changeUserStatus(row),
  //     variant: "secondary" as const,
  //   },
  //   {
  //     label: (

...
import {
  useTripDocumentsQuery,
  useTripDocumentsSummaryQuery,
...
export default function DocumentsTab({ tripId }: { tripId: string }) {
  const { data: summary, isLoading: isSummaryLoading } =
    useTripDocumentsSummaryQuery(tripId);
  const { data: documents, isLoading: isDocsLoading } =
    useTripDocumentsQuery(tripId);
  const { mutateAsync: deleteDoc } = useDeleteTripDocumentMutation(tripId);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-black/20 rounded-xl p-6 shadow-sm">
        <h3 className="text-xl font-bold text-black mb-4">Trip Documents</h3>

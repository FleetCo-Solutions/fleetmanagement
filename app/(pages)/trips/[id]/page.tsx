...
export default function TripDetail() {
  ...
  const timelineData = [
    ...
    {
      time: new Date(new Date(trip.startTime).getTime() + 2 * 60 * 60 * 1000).toISOString(),
      event: 'Checkpoint 1',
      location: 'Dodoma',
      status: 'completed',
    },
    {
      time: new Date(new Date(trip.startTime).getTime() + 4 * 60 * 60 * 1000).toISOString(),
      event: 'Checkpoint 2',
      location: 'Morogoro',
      ...
      <div className="w-[96%] h-[96%] flex flex-col gap-6 overflow-auto">
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-black/60">{trip.startLocation} → {trip.endLocation}</p>
            </div>
          </div>
          <div className="flex gap-3">
            <span className={`px-3 py-2 rounded-full text-sm font-semibold ${getStatusColor(trip.status)}`}>
            ...
            <div className="bg-white border border-black/20 rounded-xl p-6 shadow-sm h-full">
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-black/70">{trip.distance.toLocaleString()} km</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  <span className="font-semibold text-black/70">Fuel Used:</span>
                  <span className="text-black/70">{trip.fuelUsed} L</span>
                </div>
                <div className="flex justify-between">
                  <span className="font-semibold text-black/70">Start Time:</span>
                  <span className="text-black/70">{new Date(trip.startTime).toLocaleString('en-GB')}</span>
                </div>
                {trip.endTime && (
                  <div className="flex justify-between">
                    <span className="font-semibold text-black/70">End Time:</span>
                    <span className="text-black/70">{new Date(trip.endTime).toLocaleString('en-GB')}</span>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white border border-black/20 rounded-xl p-6 shadow-sm">
            <h2 className="text-xl font-bold text-black mb-4">Real-Time Data</h2>
            <div className="space-y-4">
              <div className="flex justify-between">
                <span className="font-semibold text-black/70">Current Location:</span>
                <span className="text-black/70">{realTimeData.currentLocation}</span>
              </div>
              <div className="flex justify-between">
                <span className="font-semibold text-black/70">Current Speed:</span>
                <span className="text-black/70">{realTimeData.currentSpeed} km/h</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                <span className="font-semibold text-black/70">Engine Temp:</span>
                <span className="text-black/70">{realTimeData.engineTemp}°C</span>
              </div>
              <div className="flex justify-between">
                <span className="font-semibold text-black/70">Last Updated:</span>
                <span className="text-sm text-black/70">{realTimeData.lastUpdated}</span>
              </div>
            </div>
          </div>
              ...
              {timelineData.map((item, index) => (
                <div key={index} className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="font-semibold text-black/70">{item.event}</div>

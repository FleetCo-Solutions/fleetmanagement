...
export const VehicleSchema = z.object({
  ...
  registrationNumber: z.string().max(15).openapi({
  ...
  model: z
    .string()
    .max(30)
    .openapi({ description: "Vehicle model", example: "Model X" }),
  manufacturer: z
    .string()
    .max(30)
    .openapi({ description: "Vehicle manufacturer", example: "Tesla" }),
  vin: z.string().length(17).openapi({
  ...
  color: z
    .string()
    .max(20)
    .openapi({ description: "Vehicle color", example: "Red" }),
  createdAt: z
    .string()
    .datetime()
  ...
  drivers: z
    .array(z.any())
    .optional()
    .openapi({ description: "Associated drivers" }),
});
...
export const CreateVehicleRequestSchema = z.object({
  vehicleRegNo: z.string().min(1).max(15).openapi({
    description: "Vehicle registration number",
    example: "ABC-123",
  }),
  vin: z.string().length(17).openapi({
